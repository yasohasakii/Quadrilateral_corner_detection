...
class DataParser():
    ...
    def __init__(self, cfgs):
        ...
        ####
        self.training_triads = self.io.read_file_list2(self.train_file)
        self.samples = self.io.split_triads(self.training_triads, self.train_data_dir)
        ###
        self.io.print_info('Training data set-up from {}'.format(os.path.join(self.train_file)))
        # self.n_samples = len(self.training_pairs)
        self.n_samples = len(self.training_triads)
        ...
        self.training_ids = self.all_ids[:int(self.cfgs['train_split'] * len(self.training_triads))]
        self.validation_ids = self.all_ids[int(self.cfgs['train_split'] * len(self.training_triads)):]
    ...
    def get_batch(self, batch):
        ...
        for idx, b in enumerate(batch):
            ...
            sz = np.array(self.samples[b][2],dtype=np.int32)
            cd = np.array(self.samples[b][3], dtype=np.float32)

            # attention here! all images must be resized to a fixed size: w x h by config file defined.
        ...
        return images, edgemaps, sizes, coordinates, filenames

    # for backup
    def get_batch_bak(self, batch):
...
if __name__ == '__main__':
    config_file = '/mnt/hgfs/Project/local_repository/jd-edge/hed/configs/hed.yaml'
    pfile = open(config_file)
    cfgs = yaml.load(pfile)
    pfile.close()
    data_gen = DataParser(cfgs=cfgs)
    print()
